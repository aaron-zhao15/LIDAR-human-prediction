...
public:
    ...
    ~DigitState();
    // ALL world frame except for the generalized coordinates
    VectorXd base_position = VectorXd::Zero(3);  
    VectorXd base_orientation = VectorXd::Zero(3); //[roll,pitch,yaw] angles in radians this is filtered one but if filter is off, it is unfil
